...
{
	/**
	 * Renders the upcoming birthday on the dashboard
	 *
	 * @since	2.0
	 * @access	public
	 */
	public function sidebarBottom()
	{
		...
		$birthdays = $this->getUpcomingBirthdays($key, $this->my->id);
		...
		foreach ($birthdays as $birthday) {
			...
			if ($displayYear) {
				...
				if (!$privacy->validate('field.birthday.year', $birthday->field_id, 'year', $birthday->uid)) {
	...
	public function getUpcomingBirthdays( $key , $userId )
	{
		...
		if ($this->config->get('friends.enabled')) {
			// join with friends only if Friends system enabled.
			$query .= ' INNER JOIN (';
		...
		$query .= ' and a.`datakey` = ' . $db->Quote('date');
