...
EasySocial.Controller('Conversations.Item', {
	...
	"{item} click" : function(el, event) {
		...
		EasySocial.ajax( 'site/views/conversations/getConversation',{
			"id"   : id,
			"isloadmore" : 0
		}).done(function(title, messages, lastupdate, actions, lastCreatorUserEmail, lastCreatorExist) {

			var arguments = [title, messages, lastupdate, actions, lastCreatorUserEmail, lastCreatorExist];
