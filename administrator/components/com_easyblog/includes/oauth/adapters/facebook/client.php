...
class EasyBlogClientFacebook extends EasyBlogFacebookConsumer
{
	...
	{
		$scopes = array('manage_pages', 'publish_pages', 'publish_to_groups');
	...
	public function extractPostData(EasyBlogPost &$post)
	{
		...
		if ($max && (EBString::strlen($data->content) > $max)) {
			$data->content = EBString::substr($data->content, 0, $max) . JText::_('COM_EASYBLOG_ELLIPSES');
		}

		$url = 'index.php?option=com_easyblog&view=entry&id=' . $post->id;

		// Get the url to the blog
		$data->url = EBR::getRoutedURL($url, false, true);
