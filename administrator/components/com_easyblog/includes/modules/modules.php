...
class EasyBlogModules extends EasyBlog
{
	...
	public function processItems($data)
	{
		...
		foreach ($posts as $post) {
			...
			// Loadmodule tag should not appear in the module or it will be an inception of modules. #900
			$post->summary = EB::post()->removeLoadmodulesTags($post->summary);
			...
			if ($stripTags || $autoTruncate) {
				...
				if ($media->images) {
					...
					// remove image caption before we strip tags #1067, #1548
					$pattern = '/<div class="eb-image-caption">[\n|\t]+<span>(.*)<\/span>[\n|\t]+<\/div>/i';
