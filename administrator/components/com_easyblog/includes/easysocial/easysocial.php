...
class EasyBlogEasySocial extends EasyBlog
{
	...
	public function getCommentHTML($blog)
	{
		...
		$comments = ES::comments($blog->id, 'blog', SOCIAL_APPS_GROUP_USER, $url);
		...
		if (!$isNew) {
			...
			if ($contribution && in_array($contribution->type, $esClusterType)) {
				...
				if (count($streams) > 0) {
					...
					$model = EB::model('Blog');
					$model->updateStreamContribution($streamIds, $contribution);
				}
			}
		}
	...
	public function notifySubscribers(EasyBlogPost $blog, $action, $comment = null)
	{
		...
		$permalink = $blog->getPermalink(true, false, null, true);

		if ($comment) {
			$commentLink = $blog->getExternalBlogLink('index.php?option=com_easyblog&view=entry&id='. $blog->id) . '#comment-' . $comment->id;
	...
	public function getToolbarDropdown()
	{
		...
		$output = $theme->output('site/easysocial/toolbar');
