...
class EasyBlogModelBlog extends EasyBlogAdminModel
{
	...
	*/
	...
	public function getTaggedBlogs($tagId = 0, $limit = false, $includeCatIds = '', $sorting = '', $excludeFeatured = false, $options = array())
	{
		...
		if (EB::jomsocial()->exists() && $config->get('main_jomsocial_privacy') && !EB::isSiteAdmin()) {
	 ...
	 * @access  public
	 */
	public function updateStreamContribution($streamIds = array(), $contribution)
	{
		$db = EB::db();

		$streamIds = implode(',', $streamIds);
		$query = 'UPDATE ' . $db->nameQuote('#__social_stream') . ' '
				. 'SET ' . $db->nameQuote('cluster_id') . '=' . $db->Quote($contribution->id) . ', '
				. $db->nameQuote('cluster_type') . '=' . $db->Quote($contribution->type) . ', '
				. $db->nameQuote('cluster_access') . '=' . $db->Quote(1) . ' '
				. 'WHERE ' . $db->nameQuote('id') . ' IN (' . $streamIds . ')';
		$db->setQuery($query);
		$db->query();
	}
