...
class EasySocialModelSearchCluster extends EasySocialModel
{
	...
	public function buildAdvSearch($match, $options)
	{
		...
		if (isset($options['clusterCategoryIds']) && $options['clusterCategoryIds']) {
			$query .= ' AND u.`category_id` IN (' . implode(',', $options['clusterCategoryIds']) . ')';
		}
	...
	private function genClusterPrivacy($userId, $type)
	{
		...
		if ($userId) {
			$query .= ' and (u.`type` IN (' . $private . ', ' . $public . ')';
