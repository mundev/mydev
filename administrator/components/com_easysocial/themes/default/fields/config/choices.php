...
>
	<?php if (!empty($value)) { ?>
		<?php foreach ($value as $key => $v) { ?>
			<?php echo $this->loadTemplate('admin/fields/config/choices.item', array('id' => $key, 'title' => $v->label, 'value' => $v->value, 'default' => isset($v->default) ? $v->default : 0)); ?>
		<?php } ?>
	<?php } else { ?>
		<?php echo $this->loadTemplate('admin/fields/config/choices.item', array('id' => 0, 'title' => '', 'value' => '', 'default' => 0)); ?>
	<?php } ?>
</ul>
