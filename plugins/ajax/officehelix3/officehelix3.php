...
class plgAjaxOfficeHelix3 extends JPlugin
{
	function onAjaxHelix3()
	{
		...
		if ($_POST['data']) {
			...
			switch ($action) {
				...
				$plugin_path   = JPATH_PLUGINS . '/system/officehelix3/assets/webfonts/webfonts.json';
				...
				if ($image['error'] == UPLOAD_ERR_OK) {
					...
					if(!$error) {
						...
						if(JFile::upload($path, $dest)) {
							...
							$sources = OfficeHelix3Image::createThumbs($dest, $sizes, $folder, $base_name, $ext);
