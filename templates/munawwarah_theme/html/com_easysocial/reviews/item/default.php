...
<div class="es-container" data-review-item data-uid="<?php echo $cluster->id;?>" data-type="<?php echo $cluster->getType();?>" data-id="<?php echo $reviews->id;?>">
	...
	<div class="es-content">
		...
		<?php if (isset($cluster) && $cluster) { ?>
			<?php echo $this->html('cover.' . $cluster->getType(), $cluster, 'reviews'); ?>

			<?php echo $this->html('html.snackbar', 'APP_REVIEWS_' . $cluster->getType() . '_TITLE' , 'h2'); ?>
		<?php } ?>

		<div class="es-entry-actionbar es-island">
			<div class="o-grid-sm">
				<div class="o-grid-sm__cell">
					<a href="<?php echo $cluster->getAppPermalink('reviews'); ?>" class="btn btn-es-default-o btn-sm">&larr; <?php echo JText::_('APP_REVIEWS_BACK_TO_REVIEWS'); ?></a>
				</div>

				<?php if ($cluster->isAdmin() || $this->my->isSiteAdmin() || $reviews->created_by == $this->my->id) { ?>
				<div class="o-grid-sm__cell">
					<div class="btn-group pull-right" role="group">
						...
						<ul class="dropdown-menu dropdown-menu-user messageDropDown">
							...
							<?php if ($reviews->isPending() && $cluster->isAdmin()) { ?>
								<li class="divider"></li>
								<li>
