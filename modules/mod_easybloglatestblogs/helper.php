...
class modLatestBlogsHelper extends EasyBlog
{
	...
	public function getPosts($id = null, $type = 'latest')
	{
		...
		if ($type == 'blogger') {
			$posts = $model->getBlogsBy('blogger', $id, $sort, $count, EBLOG_FILTER_PUBLISHED, null, $respectFrontpage, $excludeBlog, false, false, true, array(), array(), null, 'listlength', true, array(), array(), $excludeFeatured);
		}

		if ($type == 'category') {
			$posts = $model->getBlogsBy('category', $id, $sort, $count, EBLOG_FILTER_PUBLISHED, null, $respectFrontpage, $excludeBlog, false, false, true, array(), array(), null, 'listlength', true, array(), array(), $excludeFeatured);
		}
		...
		if ($type == 'team') {
			$posts = $model->getBlogsBy('teamblog', $id, $sort, $count, EBLOG_FILTER_PUBLISHED, null, $respectFrontpage, $excludeBlog, false, false, true, array(), array(), null, 'listlength', true, array(), array(), $excludeFeatured);
		}
		...
		if ($type == 'latest') {
			...
			if ($featuredOnly) {
				...
				$posts = $model->getBlogsBy($type, $cid, array($sort, 'DESC'), $count, EBLOG_FILTER_PUBLISHED, null, $respectFrontpage, $excludeBlog, false, false, true, array(), $cid, $postType, 'listlength', true, array(), array(), $excludeFeatured);
			}
		}
