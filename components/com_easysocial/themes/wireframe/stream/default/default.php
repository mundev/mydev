...
<div class="es-streams <?php echo !is_array($streams) ? ' is-empty' : ''; ?>" data-es-streams data-cluster="<?php echo $cluster ? '1' : '0';?>" data-currentdate="<?php echo ES::date()->toSql(); ?>" data-excludeids data-identifier="<?php echo $identifier; ?>">
	...
	<div class="es-stream-pinned-divider <?php echo !$stickies ? 't-hidden' : ''; ?>" data-stream-sticky-divider><span><i class="fa fa-thumb-tack"></i> <?php echo JText::_('COM_EASYSOCIAL_STREAM_PINNED_ITEMS'); ?></span></div>
	...
	<?php if (!$this->my->guest) { ?>
		<div class="es-pagination">
			<?php if ($nextlimit) { ?>
				<a href="javascript:void(0);" class="btn btn-es-default-o btn-block" data-pagination data-customlimit="<?php echo $customlimit;?>" data-nextlimit="<?php echo $nextlimit;?>" data-context="<?php echo $this->html('string.escape', $context);?>">
